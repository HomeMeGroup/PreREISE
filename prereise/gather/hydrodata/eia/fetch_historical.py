...
def get_generation(api_key, year, regions=None, acceptable_missing=24, verbose=False):
    ...
    :param iterable regions: subset of regions to fetch & return. If None, all will be
        returned.
    :param int acceptable_missing: allowable number of missing entries, will be linearly
        imputed. If more than this number are missing in the given ``year``, the region
        will be dropped.
    :param bool verbose: whether or not to report the number of missing for each
        excluded region.
